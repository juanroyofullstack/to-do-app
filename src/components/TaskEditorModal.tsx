        ...
        <div className='TaskEditorModal flex absolute items-center justify-center w-full h-full'>
            <form onSubmit={handleModifyTask} className="flex justify-center flex-col rounded-md z-10 gap-4 border-gray-200 bg-white p-20 w-6/12">
                ...
                <div className='flex justify-center gap-4'>
                    <CustomButton title={'Save'} className='button bg-slate-200 rounded-md shadow-md p-2' type="submit" />
                    <CustomButton title={'Delete'} className='button bg-slate-200 rounded-md shadow-md p-2' onClick={() => handleDeleteTask()} />
                    <CustomButton title={'Cancel'} className='button bg-slate-200 rounded-md shadow-md p-2' onClick={() => setAppState({modifyState: false, taskData: undefined})} />
                </div>
